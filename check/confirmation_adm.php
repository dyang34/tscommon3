<? 
	include ("../include/top_admin.php"); 
	if(!$_SESSION['s_admin_id']){
?>
<script>
	alert('로그인이 필요합니다.');
	window.close();
...
if ($row_trip['company_type']=="1") {
	$member_no=$row_trip['member_no'];
	include_once $root_dir."/config/get_site_config_member_no.php";

	$member_no = $site_config_member_no;

} else if ($row_trip['company_type']=="2") {
	$member_no = 20000;
}
...
<?
	...
	while($row_mem=mysql_fetch_array($result_mem)) {
		...
		} elseif ($sex_type=="5" || $sex_type=="6") {
			$year_text="19".decode_pass($row_mem['jumin_1'],$pass_key);
		} elseif ($sex_type=="7" || $sex_type=="8") {
			$year_text="20".decode_pass($row_mem['jumin_1'],$pass_key);
		}
?>
                            <tr>
...
<script>
	function view_pop(code) {
		...
		$.ajax({
			type : "POST",
			url : "../src/plan_code_view_adm.php",
			data :  { "code" : code , "trip_type" : "<?=$row_trip['trip_type']?>" , "select_member_no" : "<?=$member_no?>", "company_type" : "<?=$row_trip['company_type']?>", "auth_token" : auth_token },
					...
					$.each(json.msg,function(key,state){
						if (i_cnt==0) {
							add_html = add_html+"<tr>"+
								"<td rowspan=\"34\">"+json.msg_title+"<br/>("+code+")"+"</td>"+
