...
?>
<script type="text/javascript" src="../js/trip.js?ver=<?=date("Ymdhis")?>"></script>
<script type="text/javascript" src="../js/jquery.bxslider.min.js?ver=<?=date("Ymdhis")?>"></script>
<link rel="stylesheet" type="text/css" href="../css/jquery.bxslider.css?ver=<?=date("Ymdhis")?>">
		 ...
		 var div = '<div class="infoform  infoform-add">';
				...
				div += '<div class="col-sm-1">';
					div += '<div class="select_ds"';
						...
						div += '<input class="input" name="nation_name[]" placeholder="미국" type="text">';
					div += '</div>';
				div += '</div>';
...
/*
				div += '<div class="step_select two">';
					div += '<div class="select_ds cb">';
						div += '<label class="ss_tit db">휴대폰 번호</label>';
					...
					div += '</div>';
					div += '<div class="select_ds">';
						div += '<label class="ss_tit db">이메일</label>';
						div += '<div class="col-sm-2">';
...
/*
<?
if($thai_chk == 'indonPass' && $nation_chk == 'indonesia'){
?>
			if (days < 46)
			{
				alert ('고객님은 태국 46일 이상 방문을 선택하셨습니다. \n 46일 이상으로 설정해 주세요');
			}
<?}?>
...
?>
			if (days >= 45)
			{
				alert ('고객님은 태국 45일 이하 방문을 선택하셨습니다. \n 45일 이하로 설정해 주세요');
			}
<?}?>
*/
/*
<?
if($thai_chk == 'thaiPass'){
?>

					day_new = new Date($("#start_date").val());
					day_new_1 = day_new.setDate(day_new.getDate()+44);
					day_new_2 = new Date(day_new_1);
					day_new_3 = day_new_2.toLocaleDateString();

					var user = navigator.userAgent; 
					if( user.indexOf("iPhone") > -1 || user.indexOf("Android") > -1 ) {

						day_new_4 = day_new_3.replace(" ","");
						day_new_4 = day_new_3.replace(/[년]/g,'-');
						day_new_4 = day_new_4.replace(/[월]/g,'-');
						day_new_4 = day_new_4.replace(/[일]/g,'');
						day_new_4 = day_new_4.replace(" ","");
						day_new_4 = day_new_4.replace(" ","");
						day_new_4 = day_new_4.replace(".","-");
						day_new_4 = day_new_4.replace(".","-");
						day_new_4 = day_new_4.replace(".","-");
						day_new_4_arr =  day_new_4.split ("-");

						if (day_new_4_arr[1]<10)
						{
							day_new_4_arr[1]='0'+day_new_4_arr[1];
						}
						if (day_new_4_arr[2] < 10) 
						{
							day_new_4_arr[2]='0'+day_new_4_arr[2];
						}
							day_new_4 = day_new_4_arr[0]+'-'+day_new_4_arr[1]+'-'+day_new_4_arr[2];
						if (parseInt(date_term)<46) {
							alert('태국의 경우 최소 45일 이상 보험가입을 하셔야 합니다. 귀하의 45일째 일자는'+day_new_4+ '일 24시 입니다.\n문제 발생시 크롬이나 모바일로 이용 부탁드립니다.');
							$("#end_date").val(day_new_4);
							$("#end_hour").val(24);
						}
					...
					if( navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1 || (agent.indexOf("msie") != -1)) {
						...
						day_new_4 = day_new_4_arr[0]+'-'+day_new_4_arr[1]+'-'+day_new_4_arr[2];
//						day_new_4 = day_new_4.substr(0,10);
//						alert(day_new_4);
					...
					if (parseInt(date_term)<ie_num) {
						alert('태국의 경우 최소 45일 이상 보험가입을 하셔야 합니다. 귀하의 45일째 일자는'+day_new_4+ '일 24시 입니다.\날짜변경을 해 주세요.');
						$("#end_date").val('');
						$("#end_hour").val('24');
						$("#ie_type").val('no');
					}
						...
						day_new_2 = new Date(day_new_1);
						day_new_3 = day_new_2.toLocaleDateString();
						day_new_4 = day_new_3.replaceAll(" ","");
						day_new_4 = day_new_4.replaceAll(".","-");
						day_new_4_arr =  day_new_4.split ("-");
						if (day_new_4_arr[1] < 10)
						{
							day_new_4_arr[1]='0'+day_new_4_arr[1]
...
//					day_new_4_arr =  day_new_4.split ("-");
//					day_new_4 = day_new_4_arr[0]+'-'+day_new_4_arr[1]+'-'+day_new_4_arr[2];
//					alert(day_new_4_arr[0]);
//					alert(day_new_4_arr[1]);
//					alert(day_new_4_arr[2]);
...
/*
				if(ncode == '486') {
					...
					alert('태국(타이) 입국 조건부 승인에 따른 여행자보험 가입 조건 안내 (적용기간 : 2020.3.10 ~ 2020.6.9)\n\n- 태국내에서 최소 10만불이 보상될 수 있는 여행자보험 \n  서류를 제출해야함.\n- 태국 방문 시 조건부 승인 전용 여행자보험 플랜으로 \n  가입 요청은 1800-9010으로 연락주세요.');
...
/*
				if(ncode == '455') {
					...
					alert('인도네시아 입국 조건부 승인에 따른 여행자보험 가입 조건 안내 \n\n- 인도네시아에서 최소 10만불이 보상될 수 있는 여행자보험 \n  서류를 제출해야함.\n- 인도네시아 방문 시 조건부 승인 전용 여행자보험 플랜으로 \n  가입 요청은 1800-9010으로 연락주세요.');
					//$('input[name=nation]').val(ncode);
					$('input[name=nation_search]').val('');
					$("#search_window").hide();	
					$("#com_search_val").empty();
...
<? } ?>
						...
						<p class="ico_ceo"><span>대표피보험자</span></p>
							<div class="infoform">
								<div class="step_select two">
									<div class="select_ds cb">
										<label class="ss_tit db"><strong class="point_c">이름 / 영문이름 <br />(한글이름 필수 / 영문증서 필요시 영문명 추가 기입바랍니다.) *</strong></label>
										<input class="input" name="input_name[]" placeholder="홍길동 / Home gil dong" type="text">
									</div>
									<div class="select_ds">
										...
										<div class="col-sm-1">
											<div class="select_ds" id="f_country_show" style="display:none;">
												<span class="caution">영주권자 및 이중국적자의 경우 해당 국가를 입력하여 주시기 바랍니다. (필수)</span>
												<input class="input" name="nation_name[]" id="f_conutry" placeholder="미국" type="text">
											</div>
										</div>
									</div>
								</div>
