...
<?
					...
					$sql_check="select * from hana_plan_tmp where session_key='".$_POST['param_opt_1']."' and order_type='2'";
...
<?
						...
						$sql_tmp="insert into hana_plan set
									...
									join_name='".$row_check['join_name']."',
									join_hphone='".$row_check['join_hphone']."',
									order_no='".$ordr_idxx."',
									card_cd='".$card_cd."',
									card_name='".$card_name."',
									tno='".$tno."',
						...
						$kakao_phone="82".substr(decode_pass($row_check['join_hphone'],$pass_key), 1,10);
						
			/*
						$msg=$row_check['join_name']." 고객님! 투어세이프 입니다.
선물하기가 완료되었습니다. 감사합니다.";
*/
				//$buyr_name = 구매자, $row_check['join_name'] = 여행자, 선물 수령자(대표)
						$msg=" 안녕하세요! ".$buyr_name." 고객님, 투어세이프 여행자보험 센터입니다.
						".$row_check['join_name']." 고객님의 안전한 여행을 위한 여행자보험 선물하기가 완료되었습니다.
						선물 받으신 고객님이 여행 출발 전까지 반드시 선물번호를 등록하셔야 
						여행자보험 보장이 가능합니다. 출발 전 까지 미 등록 시에는 자동 취소될 수 있음을 
						알려드립니다. 
						투어세이프 여행자보험 센터를 통해 여행자보험을 가입해주셔서 감사합니다.";


						sendTalk_common_gift($kakao_phone, "tour_common_gift_0708", $msg,"https://"._TOURSAFE_SUBSITE_NAME.".toursafe.co.kr/gift/02.php");
						...
						while($row_mem=mysql_fetch_array($result_mem)) {
							...
							$gift_key="G".date("ymd").sprintf("%06d", $row_mem['no']);
						...
						$sql_delete="delete from hana_plan_tmp where session_key='".$_POST['param_opt_1']."'";
...
?>
<script>
	location.href="../gift/complete.php?check=<?=$_POST['param_opt_1']?>";
</script>
<?
						exit;
					}
...
<script>
	...
	location.href="../gift/step05_mobile.php";
</script>
<?
			exit;
        }
